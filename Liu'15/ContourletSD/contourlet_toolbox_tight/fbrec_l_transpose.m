function [y0, y1] = fbrec_l_transpose(x, f, type1, type2, extmod)
% FBDEC_L   Two-channel 2D Filterbank Decomposition using Ladder Structure as the Reconstruction transposition
%
...
%y0 = sqrt(2) * p0 + sefilter2(p1, f, f, extmod, [1, 1]);
%y1 = (-1 / sqrt(2)) * (p1 + sefilter2(y0, f, f, extmod));
y1 = (-1 / sqrt(2)) * (p1 + sefilter2(p0, f, f, extmod));
y0 = sqrt(2) * p0 + sefilter2(y1, f, f, extmod, [1, 1]);