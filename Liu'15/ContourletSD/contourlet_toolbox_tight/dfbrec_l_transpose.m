function y = dfbrec_l_transpose(x, f, n)
% DFBDEC_L   Directional Filterbank Decomposition using Ladder Structure as the Reconstruction transposition
%
%	y = dfbdec_l(x, f, n)
%
% Input:
...
if n == 1
    ...
    [y{1}, y{2}] = fbrec_l_transpose(x, f, 'q', '1r', 'qper_col');

else
    % For the cases that n >= 2
    % First level
    [x0, x1] = fbrec_l_transpose(x, f, 'q', '1r', 'qper_col');

    % Second level
    y = cell(1, 4);
    [y{2}, y{1}] = fbrec_l_transpose(x0, f, 'q', '2c', 'per');
    [y{4}, y{3}] = fbrec_l_transpose(x1, f, 'q', '2c', 'per');
    ...
    for l = 3:n
        ...
        for k = 1:2^(l-2)
            i = mod(k-1, 2) + 1;
            [y{2*k}, y{2*k-1}] = fbrec_l_transpose(y_old{k}, f, 'p', i, 'per');
        end
