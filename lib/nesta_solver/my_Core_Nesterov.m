function [xk,niter,residuals,outputData,opts] = my_Core_Nesterov(...
...
%               opts.TypeCon - type of constraints, which can be either 'l2'
%                   (default), or 'linf' (similar to Dantzig Selector)
...
if ~isempty(AAtinv)
    disp('entering AAtinv condition in core')
...
if strcmpi(TypeMin,'TV')
    Lmu = 8*Lmu; % 8 should be ~ norm(TV1,2)^2
    ...
    D = sparse([Dh;Dv]);
elseif strcmpi(TypeMin,'TV2') 
    Lmu = 32*Lmu; % 31 should be ~ norm(TV1,2)^2
    D = opts.D;
elseif strcmpi(TypeMin,'TV2_diag')
    Lmu = 32*Lmu; % 31 should be ~ norm(TV1,2)^2
    D = opts.D;
elseif strcmpi(TypeMin,'TV2_cartesian')
    % use normU specified in input
    D = opts.D;
else
    error('Core: wrong Typemin')
end
...
for k = 0:maxiter-1,
    ...
    if strcmpi(TypeMin,'tv2') || strcmpi(TypeMin,'tv2_diag') || strcmpi(TypeMin,'tv2_cartesian')
        [df,fx] = Perform_TV_Constraint(xk,mu,[],[],D,U,Ut); 
    end
    
    if strcmpi(TypeCon,'L2') || delta == 0
        %% ---- Updating yk
    ...
    elseif  strcmpi(TypeCon,'Linf')
        %% ---- Updating yk
        % min_|Ax-y|<=eps  L/2 || x - xk ||_2^2 + df' x
        yk = xk - df/Lmu;
        
        yk(opts.samples) = max(bm, yk(opts.samples));
        yk(opts.samples) = min(bp, yk(opts.samples));
        A_yk = yk(opts.samples);
        
        residuals(k+1,1) = norm( b-A_xk, Inf);    % the residual
        residuals(k+1,2) = fx;              % the value of the objective
        ...
        fmean = [fx,fmean];
        if (length(fmean) > 10) fmean = fmean(1:10);end % short term memory
        
        %% --- Updating zk
        alpha_k = 0.5*(k+1);
        tau_k = 2/(k+3);
        wk =  alpha_k*df + wk; % cumulative gradient
        
        zk = xplug - wk/Lmu;
        zk(opts.samples) = max(bm, zk(opts.samples));
        zk(opts.samples) = min(bp, zk(opts.samples));
        Azk = zk(opts.samples);
    else
        error('wrong typeCon selection')
    end
...
function [df,fx] = Perform_TV_Constraint(xk,mu,Dv,Dh,D,U,Ut)
    ...
    w = max(mu,tvx);
    u = Dx ./ w;
    fx = real(u'*Dx - mu/2 * 1/numel(u)*sum(u'*u));
    if isa(Ut,'function_handle')
        df = Ut(D'*u);
    else
        df = U'*(D'*u);
