function [xk,niter,residuals,outputData,timeNesta,opts] = myNESTA(A,At,b,muf,delta,opts)
...
switch lower(TypeMin)
    ...
    case 'tv'
        mu0 = ValMUTv(Ux_ref);
    case 'tv2'
        mu0 = max(abs(opts.D * Ux_ref));
    case 'tv2_diag'
        mu0 = max(abs(opts.D * Ux_ref));
    case 'tv2_cartesian'
        mu0 = max(abs(opts.D * Ux_ref));
    otherwise
        error(['NESTA: wrong TypeMin: ', TypeMin])
end

% Dhandle = @(z)  opts.D * z;
% Dthandle = @(z)  opts.D' * z;
% [normD,cntD] = my_normest(Dhandle,Dthandle,length(xplug),1e-3,100)

% -- If U was set by the user and normU not supplied, then calcuate norm(U)
if U_userSet && isempty(normU)
    disp('entering normU condition in myNESTA')
...
niter = 0;
Gamma = (muf/mu0)^(1/MaxIntIter); % for continuation of mu
mu = mu0;
Gammat = (TolVar/0.1)^(1/MaxIntIter); % adaptive stopping criterion
TolVar = 0.1;
...
for nl=1:MaxIntIter   
    ...
    [xk,niter_int,res,out,optsOut] = my_Core_Nesterov(A,At,b,mu,delta,opts);
    ...
    % LC: no stopping conditions here??
end
opts = optsOut;
timeNesta = toc(timeNestaTic);
...
function th=ValMUTv(x)
    ...
    % LC: our definition
    th = max(max(abs(Dhx)),max(abs(Dvx)));   
    % NESTA version:
